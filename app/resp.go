package main

import (
	"bufio"
	"errors"
	"fmt"
	"io"
	"strconv"
)

const (
	STRING  = '+'
	ERROR   = '-'
	INTEGER = ':'
	BULK    = '$'
	ARRAY   = '*'
)

type Value struct {
	typ   string
	str   string
	num   int
	bulk  string
	array []Value
}

type Resp struct {
	reader *bufio.Reader
}


func NewResp(rd io.Reader) *Resp {
	return &Resp{reader: bufio.NewReader(rd)}
}


func (r *Resp) readLine() (line []byte, n int, err error) {
	for {
		b, err := r.reader.ReadByte()
		if err != nil {
			fmt.Println("Error Occured Reading Byte from reader: ", err.Error())
			return nil, 0, err
		}

		n += 1

		line = append(line, b)

		if len(line) >= 2 && line[len(line)-2] == '\r' {
			break
		}
	}
	return line[:len(line) - 2], n, nil
}


func (r *Resp) readInteger() (x int, n int, err error){
	line, n, err := r.readLine()
	if err != nil {
		fmt.Println("There was error calling readLine: ", err.Error())
		return 0, 0, err
	}

	i64, err := strconv.ParseInt(string(line), 10, 64)
	if err != nil {
		fmt.Println("Error Parsing Int: ", err.Error())
	}

	return int(i64), n, nil
}


func (r *Resp) Read() (Value, error) {
	_type, err := r.reader.ReadByte()
	if err != nil {
		return Value{}, err
	}
	
	switch _type {
	case ARRAY:
		return r.readArray()
	case BULK:
		return r.readBulk()
	default:
		fmt.Printf("Unknown type send by client: %v", string(_type))
		return Value{}, nil
	}

}


func (r *Resp) readArray() (Value, error) {
    // Skip the first byte because we have already read it in the Read method.
    // Read the integer that represents the number of elements in the array.
    // Iterate over the array and for each line, call the Read method to parse the type according to the character at the beginning of the line.
    // With each iteration, append the parsed value to the array in the Value object and return it.
    //
	
	v := Value{}
	v.typ = "array"

	_, err := r.reader.ReadByte()
	if err != nil {
		return Value{}, err
	}

	length, _, err := r.readInteger()
	if err != nil {
		return Value{}, err
	}

	v.array = make([]Value, length)
	for i:=0; i<length;i++ {
		value, err := r.Read()
		if err != nil {
			return v, err
		}

		v.array[i] = value
	}


	
	return Value{}, nil
}


func (r *Resp) readBulk() (Value, error) {
	package main

import (
	"bufio"
	"fmt"
	"io"
	"strconv"
)

const (
	STRING  = '+'
	ERROR   = '-'
	INTEGER = ':'
	BULK    = '$'
	ARRAY   = '*'
)

type Value struct {
	typ   string
	str   string
	num   int
	bulk  string
	array []Value
}

type Resp struct {
	reader *bufio.Reader
}


func NewResp(rd io.Reader) *Resp {
	return &Resp{reader: bufio.NewReader(rd)}
}


func (r *Resp) readLine() (line []byte, n int, err error) {
	for {
		b, err := r.reader.ReadByte()
		if err != nil {
			fmt.Println("Error Occured Reading Byte from reader: ", err.Error())
			return nil, 0, err
		}

		n += 1

		line = append(line, b)

		if len(line) >= 2 && line[len(line)-2] == '\r' {
			break
		}
	}
	return line[:len(line) - 2], n, nil
}


func (r *Resp) readInteger() (x int, n int, err error){
	line, n, err := r.readLine()
	if err != nil {
		fmt.Println("There was error calling readLine: ", err.Error())
		return 0, 0, err
	}

	i64, err := strconv.ParseInt(string(line), 10, 64)
	if err != nil {
		fmt.Println("Error Parsing Int: ", err.Error())
	}

	return int(i64), n, nil
}


func (r *Resp) Read() (Value, error) {
	_type, err := r.reader.ReadByte()
	if err != nil {
		return Value{}, err
	}
	
	switch _type {
	case ARRAY:
		return r.readArray()
	case BULK:
		return r.readBulk()
	default:
		fmt.Printf("Unknown type send by client: %v", string(_type))
		return Value{}, nil
	}

}


func (r *Resp) readArray() (Value, error) {
    // Skip the first byte because we have already read it in the Read method.
    // Read the integer that represents the number of elements in the array.
    // Iterate over the array and for each line, call the Read method to parse the type according to the character at the beginning of the line.
    // With each iteration, append the parsed value to the array in the Value object and return it.
    //
	
	v := Value{}
	v.typ = "array"

	_, err := r.reader.ReadByte()
	if err != nil {
		return Value{}, err
	}

	length, _, err := r.readInteger()
	if err != nil {
		return Value{}, err
	}

	v.array = make([]Value, length)
	for i:=0; i<length;i++ {
		value, err := r.Read()
		if err != nil {
			return v, err
		}

		v.array[i] = value
	}


	
	return Value{}, nil
}


func (r *Resp) readBulk() (Value, error) {
	
	// Skip the first byte because we have already read it in the Read method.
	// Read the integer that represents the number of bytes in the bulk string.
	// Read the bulk string, followed by the ‘\r\n’ that indicates the end of the bulk string.
	// Return the Value object.

	v := Value{}
	v.typ = "bulk"

	length, _, err := r.readInteger()
	if err != nil {
		return v, err
	}

	bulk := make([]byte, length)

	r.reader.Read(bulk)

	v.bulk = string(bulk)

	r.readLine()

	return v, nil
}
